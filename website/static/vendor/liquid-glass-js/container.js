class Container {
  ...
  `
    ...
    uniform vec2 u_resolution;
      ...
      // Local integration: localized pointer ripple beneath native controls.
    ...
    void main() {
      ...
      vec2 center = vec2(0.5, 0.5);
        ...
        // Local integration: keep lens strength in CSS pixels, independent of page length.
        textureCoord += totalRefraction * min(u_resolution.x, u_resolution.y) * 12.0 / textureSize;
        vec2 pointerDelta = (coord - u_pointer) * u_resolution;
        float pointerDistance = length(pointerDelta);
        float pointerFalloff = exp(-pointerDistance * pointerDistance / 18000.0);
        vec2 pointerDirection = pointerDelta / max(pointerDistance, 1.0);
        float pointerWave = sin(pointerDistance * 0.045 - u_time * 5.0);
        textureCoord += pointerDirection * pointerWave * pointerFalloff * u_pointerStrength * 9.0 / textureSize;
